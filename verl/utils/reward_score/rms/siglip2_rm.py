...
parser = argparse.ArgumentParser(description="SigLIP2 reward service.")
...
# Load SigLIP2 model for image-text reward
siglip2_device = args.device or ("cuda" if torch.cuda.is_available() else "cpu")
siglip2_model = Siglip2Model.from_pretrained(args.siglip2_model_name).to(siglip2_device).to(torch.bfloat16)
siglip2_processor = Siglip2Processor.from_pretrained(args.siglip2_model_name)
siglip2_lock = threading.Lock()
concurrency_limiter = threading.Semaphore(args.max_concurrent_queries)
print(f"SigLIP2 model loaded from {args.siglip2_model_name} on {siglip2_device}!!!")

def _compute_similarity(image_path: str, text: str) -> float:
    image = Image.open(image_path).convert("RGB")
    inputs = siglip2_processor(text=[text], images=[image], return_tensors="pt", padding=True, max_length=1024)
    inputs = {k: v.to(siglip2_device) for k, v in inputs.items()}
    with torch.no_grad():
        outputs = siglip2_model(**inputs)
    prob = torch.sigmoid(outputs.logits_per_image[0, 0])
    return float(prob.item())


def test_two_texts_one_image(
    image_path: str,
    *texts: str,
    max_length: int = 64,
) -> torch.Tensor:
    """
    Quick test: one image + N texts, return sigmoid probs per text.
    """
    if not texts:
    ...
    for idx, text in enumerate(texts):
        print(f"{probs[idx]:.1%} that image 0 is '{text}'")
    return probs
...
def siglip2_reward():
    ...
    print(f"[siglip2_reward] from={request.remote_addr} image={image_path} text='{text_preview}'")
    concurrency_limiter.acquire()
    try:
        with siglip2_lock:
            reward = _compute_similarity(image_path, text)
        print(f"[siglip2_reward] reward={reward:.6f}")
...
if __name__ == "__main__":
    ...
    #     print(f"siglip2_sim={sim:.6f}")
    #     raise SystemExit(0)
    app.run(host="0.0.0.0", port=args.port, debug=False, use_reloader=False)


# python /home/efs/hardychen/workspaces/gptoss_rlp/verl-internvl/verl/utils/reward_score/rms/siglip2_rm.py --test_image_path /home/efs/hardychen/workspaces/gptoss_rlp/InternVL/internvl_chat_gpt_oss/data/coco/train2014/COCO_train2014_000000283122.jpg --test_text "[MASK] on a showroom floor with vintage neon ads on the wall"
