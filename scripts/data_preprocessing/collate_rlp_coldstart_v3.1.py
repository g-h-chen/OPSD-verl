...
def collate_coldstart_jsonl(
    ...
    with open(output_path, "w", encoding="utf-8") as out_handle:
        for record in tqdm(_iter_jsonl(input_path), desc="Collate"):
            ...
            # v3.1: caption then reasoning
            assistant_answer = f"<think>{caption0}\n{thinking}</think>\\boxed{{{answers[0]}}}"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "internvl_chat_gpt_oss/data/rlp_coldstart/v3.1/"
            "rlp_coldstart_v3.1_collated.jsonl"
        ),
