...
def collate_ospd_v3_6_subsets(
    ...
    # ── Resume support: detect existing output and skip already-processed records ──
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    img = (extra.get("image_paths") or [""])[0]
                    q = extra.get("question", "")
                    processed_keys.add((img, q))
                    kept += 1
                    if extra.get("w_h_c"):
                        kept_a_correct += 1
                    if extra.get("wo_h_c"):
                    ...
                    pass
        if processed_keys:
            print(f"[resume] Found {len(processed_keys)} existing records in {jsonl_path}, will append")
    ...
    with open(jsonl_path, file_mode, encoding="utf-8") as jsonl_handle:
        for input_path in input_paths:
            ...
            for record in overall_bar:
                ...
                try:
                    ...
                    # Strip potentially corrupted EXIF to avoid vLLM hashing errors
                    image.info.pop("exif", None)
                except Exception:
                    continue

                if (image_path, question) in processed_keys:
                    continue
...
python /home/efs/hardychen/workspaces/gptoss_rlp/verl-internvl/scripts/data_preprocessing/collate_ospd_v3.6_subsets.py \
  ...
  --dataset docvqa \
  --reasoning \
  --data-source vlaa_thinking-docvqa \
  --split train \
...
python /home/efs/hardychen/workspaces/gptoss_rlp/verl-internvl/scripts/data_preprocessing/collate_ospd_v3.6_subsets.py \
  ...
  --dataset clevr_math \
  --reasoning \
  --data-source vlaa_thinking-clevr_math \
  --split train \
...
python /home/efs/hardychen/workspaces/gptoss_rlp/verl-internvl/scripts/data_preprocessing/collate_ospd_v3.6_subsets.py \
  ...
  --dataset arxivqa \
  --reasoning \
  --data-source vlaa_thinking-arxivqa \
  --split train \
