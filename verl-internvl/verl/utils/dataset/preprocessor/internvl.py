...
class InternVLPreprocessor(BasicPreprocessor):
    ...
    def __call__(self, messages, row_dict, add_generation_prompt=True, remove_eos_token=False):
        raw_prompt = self.processor.apply_chat_template(messages, add_generation_prompt=add_generation_prompt, tokenize=False)
        if remove_eos_token:
            raw_prompt = raw_prompt.strip().replace(self.processor.tokenizer.eos_token, "")
        multi_modal_data = {}
