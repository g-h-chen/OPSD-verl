    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch:
            ...
            for raw_prompt_ids, multi_modal_data in zip(
                ...
                if max(vllm_inputs[-1]["prompt_token_ids"]) >= 151936:
                    breakpoint()
        else:
            vllm_inputs = [
        ...
        kwargs.update({'seed': self.config.get("seed", 0)})
        ...
        with self.update_sampling_params(**kwargs):
            ...
            try:
                ...
                print(f"Error in generate_sequences: {str(e)}")
                breakpoint()
            ...
            for output_idx, output in enumerate(outputs):
                for sample_id in range(len(output.outputs)):
                    ...
                    if "response_prefix" in non_tensor_batch:
                        # let's collect the prefix+response
                        response_ids = non_tensor_batch["response_prefix"][output_idx] + response_ids
            ...
            if "response_prefix" in non_tensor_batch:
                # concet input_ids_without_response_prefix and FULL response
                seq = torch.cat([prompts.batch["input_ids_without_response_prefix"], response], dim=-1)
            else:
        ...
        if "response_prefix" in non_tensor_batch:
            # we use the position ids without response prefix
            response_position_ids = prompts.batch["position_ids_without_response_prefix"][..., -1:] + delta_position_id
            position_ids = torch.cat([prompts.batch["position_ids_without_response_prefix"], response_position_ids], dim=-1)
        else:
            response_position_ids = position_ids[..., -1:] + delta_position_id
            position_ids = torch.cat([position_ids, response_position_ids], dim=-1)
        # breakpoint()
        ...
        if "response_prefix" in non_tensor_batch:
            attention_mask = torch.cat([prompts.batch["attention_mask_without_response_prefix"], response_attention_mask], dim=-1)
        else:
            attention_mask = torch.cat([attention_mask, response_attention_mask], dim=-1)
