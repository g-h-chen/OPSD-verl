...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            teacher_module = None
            teacher_is_external = False
            opsd_teacher_sync_steps = int(self.config.actor.get("opsd_teacher_sync_steps", 1))
            opsd_teacher_ema_decay = self.config.actor.get("opsd_teacher_ema_decay", None)

            def _finalize_teacher_module(teacher_module_fsdp):
                teacher = teacher_module_fsdp
                for p in teacher.parameters():
                    p.requires_grad_(False)
                base_actor_module = self.actor_module
                if hasattr(self.actor_module_fsdp, "_fsdp_wrapped_module"):
                    base_actor_module = self.actor_module_fsdp._fsdp_wrapped_module
                if hasattr(teacher, "img_context_token_id") and hasattr(base_actor_module, "img_context_token_id"):
                    teacher.img_context_token_id = base_actor_module.img_context_token_id
                if hasattr(teacher, "conv_template") and hasattr(base_actor_module, "conv_template"):
                    teacher.conv_template = base_actor_module.conv_template
                if hasattr(teacher, "system_message") and hasattr(base_actor_module, "system_message"):
                    teacher.system_message = base_actor_module.system_message
                return teacher

            if self.config.get("teacher_model", {}).get("path", None):
                teacher_model_cfg = self.config.get("teacher_model", {})
                teacher_local_path = copy_to_local(
                    teacher_model_cfg.path, use_shm=teacher_model_cfg.get("use_shm", use_shm)
                )
                teacher_fsdp_config = teacher_model_cfg.get("fsdp_config", self.config.actor.fsdp_config)
                teacher_module_fsdp = self._build_model_optimizer(
                    ...
                    use_remove_padding=use_remove_padding,
                    use_fused_kernels=use_fused_kernels,
                    enable_gradient_checkpointing=teacher_model_cfg.get(
                        "enable_gradient_checkpointing", self.config.model.get("enable_gradient_checkpointing", False)
                    ),
                    trust_remote_code=teacher_model_cfg.get(
                        "trust_remote_code", self.config.model.get("trust_remote_code", False)
                    ),
                    use_liger=teacher_model_cfg.get("use_liger", self.config.model.get("use_liger", False)),
                    role="ref",
                    enable_activation_offload=teacher_model_cfg.get(
                        "enable_activation_offload", self.config.model.get("enable_activation_offload", False)
                    ),
                )[0]
                teacher_module = _finalize_teacher_module(teacher_module_fsdp)
                teacher_is_external = True
            elif opsd_teacher_ema_decay is not None:
                teacher_local_path = copy_to_local(self.config.model.path, use_shm=use_shm)
                teacher_fsdp_config = self.config.actor.fsdp_config
                teacher_module_fsdp = self._build_model_optimizer(
                    model_path=teacher_local_path,
                    fsdp_config=teacher_fsdp_config,
                    optim_config=None,
                ...
                teacher_module = _finalize_teacher_module(teacher_module_fsdp)
                print(f'synced module with ema decay {opsd_teacher_ema_decay}')
            elif opsd_teacher_sync_steps > 1:
                teacher_local_path = copy_to_local(self.config.model.path, use_shm=use_shm)
                teacher_fsdp_config = self.config.actor.fsdp_config
                teacher_module_fsdp = self._build_model_optimizer(
                    model_path=teacher_local_path,
            ...
            self.actor = DataParallelPPOActor(
                ...
                teacher_is_external=teacher_is_external,
            )
    ...
    def update_actor_opsd(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            if self.config.actor.get("debug_decode_opsd", False):
                try:
                    if dist.get_rank() == 0:
                        input_ids = data.batch["input_ids"][0].tolist()
                        responses = data.batch["responses"][0].tolist()
                        decoded_input = self.tokenizer.decode(input_ids, skip_special_tokens=False)
                        decoded_resp = self.tokenizer.decode(responses, skip_special_tokens=False)
                        print(f"[OPSD debug] input_ids[0]: {decoded_input}")
                        print(f"[OPSD debug] responses[0]: {decoded_resp}")
                except Exception as exc:
                    print(f"[OPSD debug] decode failed: {exc}")
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_opsd", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_opsd", logger=logger)
