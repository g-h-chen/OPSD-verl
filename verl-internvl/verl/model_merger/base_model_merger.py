...
def parse_args():
    ...
    base_op_parser.add_argument(
        "--trust_remote_code",
        action="store_true",
        help="Whether to trust custom code when loading model config and classes.",
        default=True,
    )
    base_op_parser.add_argument(
        "--tie-word-embedding",
...
class BaseModelMerger(ABC):
    ...
    def __init__(self, config: ModelMergerConfig):
        ...
        self._ensure_internvl_custom_code()
        self.model_config = AutoConfig.from_pretrained(
            self.hf_model_config_path, trust_remote_code=self.config.trust_remote_code
        )

    def _ensure_internvl_custom_code(self):
        if not self.hf_model_config_path:
            return

        config_path = Path(self.hf_model_config_path) / "config.json"
        ...
        model_type = (config_json.get("model_type") or "").lower()
        if "InternVLChatModel" not in architectures and not model_type.startswith("internvl"):
            return
        ...
        missing = [name for name in required_files if not (hf_dir / name).exists()]
        if not missing:
            return

        # repo_root = Path(__file__).resolve().parents[4]
        repo_root = Path("/home/efs/hardychen/workspaces/gptoss_rlp")
        candidate_dirs = [
            repo_root / "InternVL" / "internvl_chat_gpt_oss" / "internvl" / "model" / "internvl_chat",
            repo_root / "InternVL" / "internvl_chat" / "internvl" / "model" / "internvl_chat",
        ]
        for source_dir in candidate_dirs:
            ...
            for name in missing:
                ...
                if source_path.exists():
                    shutil.copy2(source_path, hf_dir / name)
            missing = [name for name in required_files if not (hf_dir / name).exists()]
            if not missing:
                return

        raise FileNotFoundError(
            "Missing InternVL custom code in HuggingFace config dir. "
            f"Expected files: {', '.join(missing)} under {hf_dir}. "
            "Provide these files manually or ensure the InternVL source repo is available."
        )
