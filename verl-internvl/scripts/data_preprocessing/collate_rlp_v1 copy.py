...
def get_word_ppl(sample_dict: Dict) -> List[float]:
    caption = sample_dict.get("caption", "").strip()
    ...
    end_idx = min(start_idx + caption_len, token_log_probs.shape[1])
    caption_log_probs = token_log_probs[0, start_idx:end_idx]
...
def select_key_span_ppl(
    ...
    candidate_indices = [
        idx
        for idx in range(1, len(word_ppl))
        if _is_concrete_word(words[idx])
        and pos_tags[idx] not in {"IN", "TO"}
        and 15 <= word_ppl[idx] <= 30
    ]
    if not candidate_indices:
        return None
    max_idx = max(candidate_indices, key=word_ppl.__getitem__)
    if not (15 <= word_ppl[max_idx] <= 30):
        return None
...
def _process_rows(
    ...
    for index, row in enumerate(tqdm(rows, desc=f"Shard {shard_id}", position=shard_id)):
        ...
        if not os.path.exists(image_path):
            continue
        sample = build_missing_word_sample(
            caption, image_path=image_path, mask_token=mask_token
        )
        if not sample:
            continue
        ...
        payload = {
            ...
            "extra_info": {
                ...
                "index": index,
                "masked_ppl": sample.get("masked_ppl"),
