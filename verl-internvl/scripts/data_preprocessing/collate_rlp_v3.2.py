...
def collate_rlp_v3(
    ...
    with open(jsonl_path, "w", encoding="utf-8") as jsonl_handle:
        for record in tqdm(_iter_jsonl(input_path), desc="Collate"):
            ...
            # v3.2: we will put the hint to model response
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Collate RLP v3.2 generations into train/val parquet."
    )
    parser.add_argument(
        "--input",
        default=(
    ...
    parser.add_argument(
        "--output",
        default="/home/efs/hardychen/workspaces/gptoss_rlp/verl-internvl/datasets/rlp/v3.2/train.parquet",
        help="Train parquet path.",
    )
    parser.add_argument(
        "--val-output",
        default="/home/efs/hardychen/workspaces/gptoss_rlp/verl-internvl/datasets/rlp/v3.2/validation.parquet",
